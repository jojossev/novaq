...
$('#chat_user').on('select2:select', function (e) {
    var user_id = e.params.data.id;
    var type = 'person';

    if (typeof loaded_chat !== 'undefined') {
        loaded_chat = {};
    }
    ...
    $("#chat-box-content").removeClass("d-none").show();
...
$('#chat_user_main').on('select2:select', function (e) {
    var user_id = e.params.data.id;   // selected user id
    var type = 'person';              // use 'person' because your switchChat checks this

    // reset previous chat if needed
    if (typeof loaded_chat !== 'undefined') {
        loaded_chat = {};
    }
