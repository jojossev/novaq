                ...
                <div class="col-12 col-sm-6">
                    <div class="col-12 text-center">
                        <div class="tab-pane active" id="pic-1">
                            <a href="<?= $product_details[0]['image'] ?>" data-toggle="lightbox"
                ...
                <div class="col-12 col-sm-6">
                    ...
                    <div class="py-2 px-3 mt-4">
                        <div class="col-md-12 mb-3">
                            <input type="text" class="kv-fa rating-loading" value="<?= $product_details[0]['rating'] ?>"
                        ...
                        if (!empty($product_details[0]['type'])) {
                            ...
                            if ($product_details[0]['type'] == 'digital_product') {
                                ?>
                                <h2 class="mb-0">
                                    <?= ($product_variants[0]['special_price'] != null && $product_variants[0]['special_price'] > 0) ? $currency . $product_variants[0]['special_price'] : $currency . $product_variants[0]['price'] ?>
                                </h2>
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($product_variants as $row) {
                                            ...
                                            ?>
                                            <tr
                                                class='<?= ($row['status'] == 7) ? "table-danger" : (($row['status'] == 0) ? "table-warning" : ""); ?>'>
                                                <td><?= $row['id'] ?>
                                                    <small><?= ($row['status'] == 7) ? "Trashed" : (($row['status'] == 0) ? "Deactived" : ""); ?></small>
                                                    <?= ($row['status'] == 7) ? "<a href='" . base_url('admin/product/change_variant_status/' . $row['id'] . '/1/' . $product_details[0]['id']) . "' title='Restore variant'>Restore</a>" : (($row['status'] == 0) ? "<a href='" . base_url('admin/product/change_variant_status/' . $row['id'] . '/1/' . $product_details[0]['id']) . "' title='Activate variant'>Activate</a>" : "<a href='" . base_url('admin/product/change_variant_status/' . $row['id'] . '/0/' . $product_details[0]['id']) . "' title='Deactivate variant'>Deactivate</a> | <a href='" . base_url('admin/product/change_variant_status/' . $row['id'] . '/7/' . $product_details[0]['id']) . "' title='Move variant to Trash'>Trash</a>") ?>
                                                </td>
                                                <td><?= str_replace(',', ' | ', $row['variant_values']) ?></td>
                                                <td><?= (($flag == 1 && isset($strike_off_price) && !empty($strike_off_price)) ? $currency . $price . ' <sup class="text-danger"><s>' . $currency . $strike_off_price . '</s></sup>' : $currency . $price) ?>
                                                </td>
                                                <td><?= isset($product_details[0]['sale_discount']) && !empty($product_details[0]['sale_discount']) && ($product_details[0]['sale_discount'] != 0) ? $sale_price : 0 ?>
                                                </td>
                                            </tr>
                                            <?php
                                            $i++;
            ...
            <div class="row mt-4">
                <nav class="w-100">
                    ...
                    <div class="nav nav-tabs" id="product-tab" role="tablist">
                        <?php if (!empty($product_details[0]['description'])) { ?><a class="nav-item nav-link active"
                                id="product-desc-tab" data-toggle="tab" href="#product-desc" role="tab"
                                aria-controls="product-desc" aria-selected="true">Description</a><?php } ?>
                        <?php if (!empty($product_rating['product_rating'])) { ?> <a
                                class="nav-item nav-link <?= $rating_active ?>" id="product-rating-tab" data-toggle="tab"
                                href="#product-rating" role="tab" aria-controls="product-rating"
                                aria-selected="false">Rating</a><?php } ?>
                        </dv>
                </nav>
                <div class="tab-content p-3 col-md-12" id="nav-tabContent">
                    <div class="tab-pane active" id="product-desc" role="tabpanel" aria-labelledby="product-desc-tab">
                        <?= $product_details[0]['description'] ?></div>
                    <?php
                    if (!empty($product_rating['product_rating'])) {
                        ?>
                        <input type="hidden" name="product_id" id="product_id"
                            value="<?= (isset($product_details[0]['id']) && !empty($product_details[0]['id'])) ? $product_details[0]['id'] : 'null' ?>" />
                        <div class="tab-pane <?= $rating_active ?>" id="product-rating" role="tabpanel"
