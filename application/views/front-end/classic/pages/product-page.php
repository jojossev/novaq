    ...
    <div class="main-content responsive-breadcrumb">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="<?= base_url('products') ?>"><?= !empty($this->lang->line('product')) ? $this->lang->line('product') : 'Product' ?></a>
                </li>
                <?php
                $cat_names = array();
                ...
                <div class="swiper-wrapper">
                    ...
                    if (isset($product['product'][0]['video_type']) && !empty($product['product'][0]['video_type'])) {
                        ...
                        <div class="swiper-slide">
                            <div class='product-view-grid'>
                                <div class='product-view-image'>
                                    <div class='product-view-image-container product-view-video'>
                                        <?php if ($product['product'][0]['video_type'] == 'self_hosted') { ?>
                                            <video controls class="product-video" src="<?= $product['product'][0]['video'] ?>">
                                        ...
                                        <?php } else if ($product['product'][0]['video_type'] == 'youtube' || $product['product'][0]['video_type'] == 'vimeo') {
                                            if ($product['product'][0]['video_type'] == 'vimeo') {
                                                ...
                                                <iframe class="product-video" src="<?= $url ?>" title="YouTube video player"
                                                    frameborder="0"
                                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture;embedded=true"
        ...
        <div class="col-12 col-md-6 product-page-details">
            ...
            <?php } else { ?>
                <div class="col-md-12 mb-3 product-rating-small  pl-0">
                    <input type="text" class="kv-fa rating-loading" value="<?= $product['product'][0]['rating'] ?>"
                        data-size="sm" title="" readonly>
                    <span class="my-auto mx-3"> ( <?= $product['product'][0]['no_of_ratings'] ?>
                        <?= !empty($this->lang->line('reviews')) ? $this->lang->line('reviews') : 'reviews' ?> ) </span>
                </div>
            <?php } ?>
            ...
            <?php }
            $color_code = $style = "";
            if (isset($product['product'][0]['variant_attributes']) && is_array($product['product'][0]['variant_attributes'])) {
                $product['product'][0]['variant_attributes'] = array_values($product['product'][0]['variant_attributes']);
            } else {
                $product['product'][0]['variant_attributes'] = [];
            }
            ...
            <div class="pt-3 text-md-left">
                ...
                <?php } else { ?>
                    <button type="button" name="add_cart" class="buttons btn-6-6 extra-small m-0 add_to_cart" id="add_cart"
                ...
                <?php if ($product['product'][0]['is_favorite'] == 0) { ?>
                    <button class="buttons btn-6-1 extra-small m-0 add-fav" id="add_to_favorite_btn"
                        data-product-id="<?= $product['product'][0]['id'] ?>">
                        <i class="fas fa-heart mr-2"></i>
                        <span><?= !empty($this->lang->line('add_to_favorite')) ? $this->lang->line('add_to_favorite') : 'Add to Favorite' ?></span>
                    </button>
                <?php } else { ?>
                    <button class="buttons btn-6-1 extra-small m-0 remove-fav" id="add_to_favorite_btn"
                        data-product-id="<?= $product['product'][0]['id'] ?>">
                        <i class="fas fa-heart mr-2"></i>
            ...
            <li>
                <div class="d-flex align-items-center"><?= label('share', 'Share') ?>: <span
                        class="ms-1 quick_view_share" id="share"></span></div>
            </li>
                                        ...
                                        if ($count <= 8) {
                                            ...
                                            <?php } else if (!empty($review_images['product_rating'][$i]['images'][$j])) { ?>
                                                    <div class="col-sm-1">
                                                        <div class="review-box">
                                                            <?php
                                                            $image = !empty($review_images['product_rating'][$i]['images'][$j])
                                                                ? $review_images['product_rating'][$i]['images'][$j]
                                                                : base_url(NO_IMAGE);
                                                            ?>

                                                            <a href="<?= $image ?>" data-lightbox="users-review-images" data-title="<?= "<button class='label btn-success'>"
                                                                  . $review_images['product_rating'][$i]['rating']
                                                                  . " <i class='fa fa-star'></i></button><br>"
                                                                  . $review_images['product_rating'][$i]['user_name']
                                                                  . "<br>"
                                                                  . $review_images['product_rating'][$i]['comment']; ?>">
                                            ...
                                            <div class="review-comment">
                                                <div class="rating-list">
                                                    <div class="product-rating">
                                                        <input type="text" readonly class="kv-fa rating-loading"
                                                            value="<?= $row['rating'] ?>" data-size="xs" title="">
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row reviews">
                                                    <?php foreach ($row['images'] as $image) { ?>
                                                        <div class="col-sm-2">
                                                            <div class="review-box">
                                                                <a href="<?= isset($image) ? $image : base_url() . NO_IMAGE; ?>"
                                                                    data-lightbox="review-images">
                                                                    <img src="<?= isset($image) ? $image : base_url() . NO_IMAGE; ?>"
                                                                        alt="Review Image">
                                                                </a>
                                                            </div>
                                                        </div>
                    ...
                    <div class="swiper-slide">
                        <div class="product-grid">
                            ...
                            <div class="col-md-12 mb-3 product-rating-small">
                                <input type="text" class="kv-fa rating-loading" value="<?= $row['rating'] ?>" data-size="sm"
                                    title="" readonly>
                            </div>
                            <div class="product-content">
                                ...
                                <a href="" class="add-to-cart add_to_cart" data-product-id="<?= $row['id'] ?>"
                                    data-product-variant-id="<?= $variant_id ?>" data-product-title="<?= $row['name'] ?>"
                                    data-product-image="<?= $row['image'] ?>" data-product-price="<?= $variant_price; ?>"
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">
                    <?= !empty($this->lang->line('add_faq')) ? $this->lang->line('add_faq') : 'Add FAQ' ?>
                </h5>
