...
<head>
    ...
    <script src="<?= base_url("assets/front_end/classic/js/jquery.min.js") ?>"></script>
    <link rel="stylesheet" href="<?= base_url('assets/front_end/classic/css/select2.min.css') ?>">
    <script src="<?= base_url("assets/front_end/classic/js/select2.full.min.js") ?>"></script>
    ...
    <script src="<?= base_url('assets/front_end/classic/js/moment.min.js') ?>"></script>

    <script src="<?= base_url('assets/front_end/classic/js/Markdown.Converter.js') ?>"></script>
    <script src="<?= base_url('assets/front_end/classic/js/Markdown.Sanitizer.js') ?>"></script>
    <script src="<?= base_url('assets/front_end/classic/js/Markdown.Editor.js') ?>"></script>
        ...
        <div class="row h-100">
            ...
            <div class="col-lg-2 floating-chat-users p-0">
                <div class="card chat-theme-light chat-scroll chat-min">
                    <select name="select_user_id[]" id="chat_user" class="search_user w-100"
                        data-placeholder=" Type to search and select users">
                        <?php
                                ...
                                <?php if (!empty($users)) {
                                    ...
                                    foreach ($users as $user) {
                                        if ($user['id'] == $_SESSION['user_id']) {
                                            ?>
                                            <li class="media p-2 mb-2 rounded hover-bg-light" style="transition: background-color 0.2s;">
                                                <div class="chat-person w-100 d-flex align-items-center" style="cursor: pointer;" data-picture="" data-type="person"
                                                    data-id="<?= $user['opponent_user_id'] ?>">
                                                    <div class="position-relative mr-3">
                                                        <?php if (!empty($user['picture'])) { ?>
                                                            <img src="<?= $user['picture'] ?>" alt="<?= $user['opponent_username'] ?>" class="rounded-circle shadow-sm" style="width: 40px; height: 40px; object-fit: cover;">
                                                        <?php } else { ?>
                                                            <div class="avatar-placeholder rounded-circle bg-primary text-white d-flex align-items-center justify-content-center shadow-sm" style="width: 40px; height: 40px; font-size: 16px; font-weight: 600;">
                                                                <?= strtoupper(substr($user['opponent_username'], 0, 1)) ?>
                                                            </div>
                                                        <?php } ?>
                                                        <i class="<?= ($user['is_online'] == 1) ? 'fa fa-circle text-success' : 'fa fa-circle text-secondary'; ?> position-absolute border border-white rounded-circle" style="bottom: 0; right: 0; font-size: 10px; padding: 1px;"></i>
                                                    </div>
                                                    <div class="media-body">
                                                        <div class="font-weight-bold text-dark mb-0" style="font-size: 14px; line-height: 1.2;">
                                                            <?= $user['opponent_username'] ?>
                                                        </div>
                                                        <div class="text-muted small mt-1">(You)</div>
                                                    </div>
                                                </div>
                                            </li>
                                        <?php }
                                    ...
                                    <p class="card-body p-0 px-5 text-muted">It seems there are no chats available at the
                                        moment</p>
                                <?php } ?>
                                                ...
                                                <div class="media-body">
                                                    <div data-unread_msg="<?= $user['unread_msg'] ?>"
                                                        ...
                                                        data-id="<?= $user['opponent_user_id'] ?>"><i
                                                            class="<?= ($user['is_online'] == 1) ? 'fa fa-circle text-success' : 'fa fa-circle'; ?> "></i>
                                                        <?= $user['opponent_username'] ?>
                            ...
                            <ul class="list-unstyled list-unstyled-border chat-list-unstyled-border">
                                ...
                                <?php if (!empty($supporters)) {
                                    ...
                                    foreach ($supporters as $supporter) {
                                        ...
                                        <li class="media">
                                            <div class="media-body">
                                                <div class="chat-person" data-id="<?= $supporter['userto_id'] ?>"
                                                    data-type="person">
                                                    <i
                        ...
                        <form id="chat-form2" autocomplete="off">
                            <div class="row">
                                <div class="input-group">
                                    ...
                                    <textarea class="form-control theme-inputs" id="chat-input-textarea" rows="1"
                                        name="chat-input-textarea"></textarea>
                                </div>
                            </div>
                            <div class="row mt-2">
