        ...
        <div class="row">
            <div class="col-md-8 mt-5 bg-white">
                <div class="cart-table-wrapper">
                    ...
                    <tbody>
<?php
$has_items = false; // ✅ ONLY ADDITION
foreach ($cart as $key => $row) {

    if (isset($row['qty']) && $row['qty'] != 0) {

        $has_items = true; // ✅ ONLY ADDITION

        if ($row['sale_price'] != 0) {
            $price = $row['sale_price'];
        } else {
            $price = $row['special_price'] != '' && $row['special_price'] != null && $row['special_price'] > 0
                ? $row['special_price']
                : $row['price'];
        }
?>
        <tr class="cart-product-desc-list">
            ...
            <td>
                ...
                <button class="btn save-for-later remove-product button button-primary button-sm"
                        data-id="<?= $row['id']; ?>">
                    <?= !empty($this->lang->line('save_for_later')) ? $this->lang->line('save_for_later') : 'Save For Later' ?>
                </button>
            </td>
            ...
            <td class="text-muted text-center p-0">
                <?= isset($row['item_tax_percentage']) && $row['item_tax_percentage'] !== '' && $row['item_tax_percentage'] !== null
                    ? $row['item_tax_percentage']
                    : '-' ?>
            </td>
                    ...
                    <?php } else { ?>
                        <div class="num-in">
                            <span class="minus dis"
                                  ...
                                  data-step="<?= (isset($row['quantity_step_size']) && !empty($row['quantity_step_size'])) ? $row['quantity_step_size'] : 1 ?>"></span>

                            <input type="text"
                                   class="in-num itemQty"
                                   data-page="cart"
                                   data-id="<?= $row['id']; ?>"
                                   value="<?= $row['qty'] ?>"
                                   data-price="<?= $price ?>"
                                   data-step="<?= (isset($row['quantity_step_size']) && !empty($row['quantity_step_size'])) ? $row['quantity_step_size'] : 1 ?>"
                                   data-min="<?= (isset($row['minimum_order_quantity']) && !empty($row['minimum_order_quantity'])) ? $row['minimum_order_quantity'] : 1 ?>"
            ...
            <td class="text-muted p-0 total-price">
                    ...
                    <div class="cart-total-price">
                        <table class="table cart-products-table">
                            <tbody>
                                ...
                                <?php if (isset($cart['total_tax_percentage'])) { ?>
                                    <tr class="cart-product-tax d-none">
                                        <td class="text-muted"><?= !empty($this->lang->line('tax')) ? $this->lang->line('tax') : 'Tax' ?> (<?= $cart['total_tax_percentage'] ?>%)</td>
