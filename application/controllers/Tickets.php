...
class Tickets extends CI_Controller
{
    ...
    public function tickets()
    {
        ...
        $this->data['tickets'] = fetch_details('tickets', ['user_id' => $_SESSION['user_id']], sort: 'tickets.id', order: 'DESC');
    ...
    public function update_ticket()
    {
        ...
        // ---------------- VALIDATION ----------------
        $this->form_validation->set_rules('edit_id', 'Ticket Id', 'trim|required|numeric|xss_clean');
        $this->form_validation->set_rules('ticket_type_id', 'Ticket Type', 'trim|required|numeric|xss_clean');
        $this->form_validation->set_rules('user_id', 'User Id', 'trim|required|numeric|xss_clean');
        $this->form_validation->set_rules('subject', 'Subject', 'trim|required|xss_clean');
        ...
        // ---------------- DATA CHECK ----------------
        $ticket = fetch_details('tickets', ['id' => $ticket_id], '*');
        if (empty($ticket)) {
            echo json_encode([
                'error' => true,
        ...
        if ($ticket[0]['user_id'] != $user_id) {
            echo json_encode([
                'error' => true,
                'message' => 'User mismatch. Cannot update ticket.',
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
            ]);
            return;
    ...
    public function send_message()
    {
        if ($this->ion_auth->logged_in()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $has_attachments = (!empty($_FILES['attachments']['name'][0]) && isset($_FILES['attachments']['name']));
                if (empty($message) && !$has_attachments) {
                    $this->response['error'] = true;
                    $this->response['message'] = "Please enter a message or upload an attachment";
                    $this->response['data'] = array();
                    print_r(json_encode($this->response));
                    return false;
                }
