...
class Webhook extends CI_Controller
{
        ...
        public function phonepe_webhook()
    {
        ...
        log_message('error', 'Request : -->' . var_export($request, true));
        // Adjust as per the actual webhook structure you receive
        $metaInfo = $request['metaInfo'] ?? [];
        log_message('error', 'Meta Info : -->' . var_export($metaInfo, true));


        $txn_id = $metaInfo['udf1'] ?? ''; //we are pass txn_id in udf1
        $amount = isset($metaInfo['udf3']) ? $metaInfo['udf3'] / 100 : null; // pass amount in udf3
        $status = $request['state'] ?? null;
