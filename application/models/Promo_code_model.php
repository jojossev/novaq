...
class Promo_code_model extends CI_Model
{
    ...
    public function get_promo_code_list($offset = 0, $limit = 10, $sort = 'id', $order = 'ASC')
    {
        ...
        if (isset($_GET['sort'])) {
            ...
            $validSortFields = ['id', 'promo_code', 'message', 'start_date', 'end_date', 'discount', 'repeat_usage', 'max_discount_amount', 'minimum_order_amount'];
            // Set sort field if it's valid
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['p.`id`' => $search, 'p.`promo_code`' => $search, 'p.`message`' => $search, 'p.`start_date`' => $search, 'p.`end_date`' => $search, 'p.`discount`' => $search, 'p.`repeat_usage`' => $search, 'p.`max_discount_amount`' => $search, 'p.`minimum_order_amount`' => $search];
        }
        ...
        // Apply sorting
        $search_res->order_by($sort, $order)->limit($limit, $offset);
        $sc_search_res = $search_res->get('promo_codes p')->result_array();
    ...
    public function add_promo_code_details($data)
    {
        ...
        if (isset($data['discount_type']) && $data['discount_type'] == 'percentage') {
            $promo_data['max_discount_amount'] = (isset($data['max_discount_amount']) && !empty($data['max_discount_amount'])) ? $data['max_discount_amount'] : '0';
        } else {
            $promo_data['max_discount_amount'] = '0';
        }

        // Handle no_of_users based on is_specific_users
        if (isset($data['is_specific_user']) && $data['is_specific_user'] == 'on') {
            // For specific users, set no_of_users to 0 or count of selected users
            $promo_data['no_of_users'] = (isset($data['users_id']) && !empty($data['users_id'])) ? count($data['users_id']) : 0;
        } else {
            // For non-specific users, use the provided no_of_users value
            $promo_data['no_of_users'] = (isset($data['no_of_users']) && !empty($data['no_of_users'])) ? $data['no_of_users'] : 0;
        }
