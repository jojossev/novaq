    ...
    function fetch_rating($product_id = NULL, $user_id = NULL, $limit = NULL, $offset = Null, $sort = Null, $order = Null, $rating_id = null, $has_images = null)
    {
        ...
        if (!empty($product_rating)) {
            ...
            for ($i = 0; $i < count($product_rating); $i++) {
                ...
                if (!isset($product_rating[$i]['user_name']) || trim($product_rating[$i]['user_name']) === '') {
                    $product_rating[$i]['user_name'] = 'Deleted User';
                }
            }
            if (!$total_images) {
    ...
    public function get_rating()
    {
        ...
        foreach ($rating_search_res as $row) {
            ...
            if ($comment_length > $max_length) {
                $short_comment = substr($comment, 0, $max_length);
                $tempRow['comment'] = '<div class="comment-container">' .
                                     '<span class="comment-text comment-text-truncated">' . $short_comment . '...' .
                                     '<span class="comment-full-text" style="display:none;">' . substr($comment, $max_length) . '</span>' .
                                     '</span>' .
                                     '<a href="javascript:void(0);" class="read-more-btn" data-action="more">Read More</a>' .
                                     '</div>';
            } else {
                $tempRow['comment'] = $comment;
            }
